...
[Orchestrator]
  classify → plan ──────────────────────────────────── → delegate → respond
              │                                               │
       [Planner Agent]                                 [CodingAssistant — 4노드]
  analyze_task → explore_context → create_plan    retrieve_memory → generate_code (STRONG)
  (REASONING)   (read-only 도구)   (구조화 계획)                      → run_tests
                                                                      → [inject_error → generate_code] × 3
```

- **Planner** (REASONING 티어): 아키텍처 설계, 페이즈 분해, 파일 구조 계획
- **CodingAssistant** (STRONG 티어): write_file 도구 루프로 코드 생성 → 테스트 실행 → 에러 시 재시도
- 원칙: Harness = 기계적 도구 제공자, LLM = 판단자 (Claude Code/Codex/DeepAgents 분석 반영)
...
| `generation` | STRONG | 코드 생성 (generate_code 노드 — write_file 도구 루프) |
| `tool_planning` | FAST | 도구 호출 판단 + ReAct 루프 |
| `verification` | DEFAULT | 코드 검증 + **대화 요약** (SummarizationMiddleware) |
| `parsing` | FAST | 요청 분류 (Orchestrator classify) |
...
### 다단계 컨텍스트 컴팩션 (Claude Code/Codex/DeepAgents 분석 반영)
- **SummarizationMiddleware** (110K): DEFAULT LLM으로 중복/반복 압축, 핵심 정보(변수명, import, 에러) 모두 보존
- **MessageWindowMiddleware** (100K): 토큰 기반 3단계 규칙 컴팩션 (마이크로→윈도우→긴급)
...
| **횡단 관심사** | 에이전트 코드에 혼재 | 양파 패턴 MiddlewareChain (Resilience → Summarization(LLM) → MessageWindow → Memory) |
