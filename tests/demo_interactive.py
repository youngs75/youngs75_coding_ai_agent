...
async def interactive_loop(client: A2AClient):
    ...
    async with httpx.AsyncClient() as http:
        while True:
            ...
            if query == "/card":
                try:
                    resp = await http.get(f"http://localhost:{args.port}/.well-known/agent-card.json")
                    card_data = resp.json()
                    print(f"\n📋 AgentCard:")
                    print(f"  Name: {card_data.get('name')}")
                    print(f"  Description: {card_data.get('description')}")
                    print(f"  URL: {card_data.get('url')}")
                    print(f"  Streaming: {card_data.get('capabilities', {}).get('streaming')}")
            ...
            if query == "/health":
                try:
                    resp = await http.get(f"{client._url or 'http://localhost:18080'}/health")
                    print(f"\n💚 {resp.json()}")
                except Exception as e:
                    print(f"  ❌ {e}")
                continue
