...
모든 주요 디렉토리에는 `AGENTS.md` 파일을 유지합니다. AI 도구(Claude Code, Codex 등)가 디렉토리 구조를 빠르게 파악하도록 돕고, 매번 파일을 전수 읽어 컨텍스트가 비대해지는 것을 방지합니다.

### 필수 포함 섹션
- **Purpose** — 이 디렉토리가 무엇을 하는지 1-2문장
- **Key Files** — 주요 파일과 역할 (테이블)
- **Subdirectories** — 하위 디렉토리와 해당 AGENTS.md 링크
- **For AI Agents** — 이 디렉토리에서 작업할 때 알아야 할 규칙/패턴

### 관리 규칙
- 새 디렉토리를 만들면 `AGENTS.md`도 함께 생성합니다.
- 파일이 추가/삭제/이름 변경되면 해당 디렉토리의 `AGENTS.md`도 업데이트합니다.
- `<!-- Parent: ../AGENTS.md -->` 주석으로 상위 문서를 참조합니다.
- `__pycache__/`, `node_modules/` 등 자동 생성 디렉토리는 제외합니다.

## 반복 작업 유틸리티 원칙
수동으로 2회 이상 반복한 작업은 유틸리티로 만들어야 합니다.

- **판단 기준**: "이 작업을 다음에도 또 수동으로 해야 하나?" → Yes이면 유틸리티 대상
- **예시**: Langfuse 트레이스 조회, E2E 세션 결과 수집, 벤치마크 비교 등
- **위치**: `coding_agent/utils/`에 독립 모듈로 생성
- **인터페이스**: CLI에서 직접 실행 가능하거나, 다른 모듈에서 import 가능한 함수 형태
- 외부 서비스(Langfuse, GitHub 등)에서 데이터를 반복 조회하는 패턴이 보이면, SDK를 활용한 프로그래밍 방식의 접근을 먼저 고려합니다
- 이 원칙은 디버깅/분석 도구에도 동일하게 적용됩니다 — 매번 웹 UI를 클릭하는 대신 스크립트로 자동화합니다
